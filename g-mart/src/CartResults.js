      ...
      {Cart.length > 0 ? (
        Cart.map((product, index) => (
          <div key={index} style={{border:'1px solid black',width:'98%',display:'flex',alignItems:'center'}} className='m-4' >
            ...
            <h1 className='tittle new-font font-bold' onClick={()=>{console.log('cart result.js : ',product.pid);navigate(`/cpdetails/${product.pid}`)}}>{product.name}</h1>
  ...
  const resultEngine = async () => {
    try {
      ...
      if(!token){
        ...
        setCartResults(data);
        console.log("Cart From Token is Sucessfull !..");
        return 1;
      }
      const response = await fetch('http://localhost:5000/api/cart');
      const data = await response.json();
